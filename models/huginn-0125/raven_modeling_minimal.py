...
class RavenForCausalLM(RavenPreTrainedModel, GenerationMixin):
    ...
    def _debug_activation_stats(self, tag: str, x: torch.Tensor, current_step: int | Tensor):
        ...
        y = x.detach()
        if y.numel() == 0:
            print(f"[act-debug] tag={tag} step={int(current_step) if not torch.is_tensor(current_step) else int(current_step.item())} empty")
            return
        ...
        step_i = int(current_step) if not torch.is_tensor(current_step) else int(current_step.item())
        print(
            f"[act-debug] tag={tag} step={step_i} "
            f"shape={tuple(x.shape)} min={min_v:.4e} max={max_v:.4e} "
            f"mean={mean_v:.4e} abs_max={abs_max:.4e} "
            f"finite={finite_all} nonfinite={nonfinite}"
        )
    ...
    def iterate_forward(
        ...
        print(f"[Huginn-debug] training={self.training} no_grad={int(num_steps_no_grad)} grad={int(num_steps_with_grad)}")
    ...
    def core_block_forward(
        ...
        self._debug_activation_stats("adapter_in", adapter_in, current_step)

        x = self.transformer.adapter(adapter_in)  # type: ignore # types broken in 2.6+
