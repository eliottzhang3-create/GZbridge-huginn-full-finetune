...
for name in MMAU_CHECKPOINTS MMAU_TEST_MINI_PATH MMAU_START_OFFSET MMAU_MAX_SAMPLES MMAU_LOG_EVERY MMAU_NUM_STEPS MMAU_MAX_NEW_TOKENS MMAU_SEED HUGINN_AUDIO_FSDP_EVAL_EXPORT_DIR; do
  if [ -n "${!name:-}" ]; then
    quote_env "$name" "${!name}"
  fi
done
