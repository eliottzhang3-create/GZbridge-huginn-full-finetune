...
WORK_ROOT="outputs/xares_voxceleb1_knn_smoke"

vc submit   -p pdgpu-4090   -i docker.v2.aispeech.com/sjtu/sjtu_wumengyue-mhl:0.0.1   -c 8 -m 32G -g 1   -n 1   -j smoke-huginn-xares-voxceleb1-knn-4090-$(date +%m%d%H%M)   -d "$SCRIPT_DIR"   JOB=1:1 "$SCRIPT_DIR/log/smoke_huginn_xares_voxceleb1_knn_4090.JOB.log"   --cmd "HUGINN_XARES_CHECKPOINT='$CHECKPOINT' HUGINN_XARES_PLUGIN_PATH='$PLUGIN_PATH' HUGINN_XARES_VOXCELEB1_ROOT='$DATA_ROOT' HUGINN_XARES_VOXCELEB1_WORK_ROOT='$WORK_ROOT' HUGINN_XARES_VOXCELEB1_USE_MINI_DATASET=1 HUGINN_XARES_VOXCELEB1_FORCE_ENCODE=1 HUGINN_XARES_VOXCELEB1_DO_KNN=1 bash scripts/run_huginn_xares_voxceleb1_knn.sh"
