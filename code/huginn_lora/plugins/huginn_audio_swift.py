...
class AudioDiscardedError(ValueError):
    """Raised when an audio sample is outside the supported duration range."""
...
def split_audio_for_whisper(
    ...
    if total_samples > discard_samples:
        raise AudioDiscardedError(
            f"Audio duration {total_samples / sample_rate:.3f}s exceeds the discard threshold "
    ...
    for start in range(0, included_samples, chunk_samples):
        ...
        if chunk.size == 0:
            continue
        # Whisper's log-mel frontend is nominally 100 frames/sec. Use only
        # complete frames before padding to the 30-second feature size so the
        # downstream 2x encoder and stride-6 compressor preserve 120ms/token.
        mel_frames = min(
            WHISPER_MAX_FEATURE_FRAMES,
            max(1, chunk.size // WHISPER_FEATURE_HOP_LENGTH),
        )
        chunks.append(chunk)
        feature_lengths.append(int(mel_frames))

    if not chunks:
        raise ValueError("Audio split produced no usable chunks")
    return chunks, feature_lengths
...
def build_huginn_audio_model(model_dir: str):
    ...
    config.audio_compressor_kernel_size = 6
