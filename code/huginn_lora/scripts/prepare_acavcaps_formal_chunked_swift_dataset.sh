...
FORMAL_CATEGORY_LIMITS="${FORMAL_CATEGORY_LIMITS:-}"
FORMAL_START_CHUNK="${FORMAL_START_CHUNK:-}"
FORMAL_END_CHUNK="${FORMAL_END_CHUNK:-}"
FORMAL_SKIP_EXISTING="${FORMAL_SKIP_EXISTING:-0}"
FORMAL_VERIFY_AUDIO_PAIRS="${FORMAL_VERIFY_AUDIO_PAIRS:-0}"

mkdir -p "$FORMAL_CHUNK_DIR"
...
if [ -n "$FORMAL_CATEGORY_LIMITS" ]; then
  ARGS+=(--category_limits "$FORMAL_CATEGORY_LIMITS")
fi
...
if [ "$FORMAL_VERIFY_AUDIO_PAIRS" = "1" ]; then
  ARGS+=(--verify_audio_pairs)
fi
...
echo "FORMAL_CATEGORY_LIMITS=${FORMAL_CATEGORY_LIMITS:-<default-subset>}"
...
echo "FORMAL_VERIFY_AUDIO_PAIRS=$FORMAL_VERIFY_AUDIO_PAIRS"
