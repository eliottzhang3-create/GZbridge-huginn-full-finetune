...
def convert_record(dataset_dir: Path, record: dict[str, Any], task: str) -> dict[str, Any]:
    audio_path = resolve_audio_path(dataset_dir, record)
    if task == "aqa":
        if "question" in record and "answer" in record:
            user_content = f"{DEFAULT_AQA_USER_PREFIX}{record['question']}"
            assistant_content = str(record["answer"])
        elif "query" in record and "response" in record:
            user_content = str(record["query"])
            assistant_content = str(record["response"])
        else:
        ...
        if "caption" in record:
            assistant_content = str(record["caption"])
...
def main():
    ...
    converted = [convert_record(dataset_dir, record, args.task) for record in records]
    ...
    print(f"[prepare-huginn-audio-dataset] samples={len(converted)} task={args.task}")
