...
DATASET_MAX_SAMPLES="${HUGINN_DYNAMIC30S_ACCEL_STAGE1_DATASET_MAX_SAMPLES:-64}"
RUN_ROOT="${HUGINN_DYNAMIC30S_ACCEL_STAGE1_RUN_ROOT:-outputs/huginn_whisper_dynamic30s_acceleration_stage1_fsdp4/run-$(date +%Y%m%d_%H%M%S)}"

if [ -e "$RUN_ROOT" ]; then
  echo "Acceleration Stage 1 run root already exists; choose a fresh HUGINN_DYNAMIC30S_ACCEL_STAGE1_RUN_ROOT: $RUN_ROOT" >&2
  exit 1
fi
mkdir -p "$RUN_ROOT"
...
FSDP_CONFIG_PATH="$RUN_ROOT/fsdp2_acceleration_stage1.json"
REPORT_PATH="$RUN_ROOT/acceleration_stage1_report.json"
...
INSPECTOR="$REPO_ROOT/code/huginn_lora/scripts/inspect_huginn_whisper_dynamic30s_acceleration_stage1.py"
...
for required_path in \
  ...
  if [ ! -e "$required_path" ]; then
    echo "Required acceleration Stage 1 path is missing: $required_path" >&2
    exit 1
  fi
done
...
if dataset_max_samples < 32:
    raise SystemExit(f"Acceleration Stage 1 requires at least 32 dataset rows, got {dataset_max_samples}")
...
if missing:
    raise SystemExit(f"Installed Swift lacks required acceleration Stage 1 arguments: {missing}")
...
print(
    "[precheck] acceleration_stage1_prerequisites=passed "
...
export HUGINN_AUDIO_DYNAMIC30S_ACCELERATION_STAGE1_AUDIT_DIR="$AUDIT_DIR"
...
echo "========== HUGINN WHISPER DYNAMIC30S ACCELERATION STAGE 1 START =========="
echo "run_root=$RUN_ROOT"
echo "scope=whisper_checkpoint_dedup_only no_attention_change=true no_reshard_change=true no_checkpoint_save=true"
...
echo "audio=retain_all_retain_first30s dynamic_160ms_tokens local_batch_longest_padding=true"
echo "fsdp_units=whisper,aligner,prelude2,recurrent_adapter_plus_core4,coda2 reshard_after_forward=true"
echo "activation_checkpointing=true vit_gradient_checkpointing=false use_reentrant=false"
echo "checkpoint_contract=outer_whisper_encoder_exactly_one internal_whisper_zero double_checkpoint_false"

TRAIN_PID=""
MONITOR_PID=""

print_resource_snapshot() {
  echo "========== DYNAMIC30S ACCELERATION STAGE1 RESOURCE SNAPSHOT =========="
...
on_exit() {
  ...
  stop_resource_monitor
  print_resource_snapshot
  echo "========== HUGINN WHISPER DYNAMIC30S ACCELERATION STAGE 1 EXIT =========="
...
echo "========== HUGINN WHISPER DYNAMIC30S ACCELERATION STAGE 1 VALIDATION PASSED =========="
