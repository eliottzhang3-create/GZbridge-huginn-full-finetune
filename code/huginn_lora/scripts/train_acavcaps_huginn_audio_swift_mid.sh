...
mkdir -p outputs/huginn_audio_acavcaps_caption_mid_generator

MID_MANIFEST="$REPO_ROOT/data/audio_swift/acavcaps/acavcaps_caption_long_mid_swift.jsonl"
PLUGIN_PATH="$REPO_ROOT/code/huginn_lora/plugins/huginn_audio_swift.py"
MODEL_PATH="$REPO_ROOT/models/huginn-audio-whisper-v1"

if [ ! -s "$MID_MANIFEST" ]; then
  echo "ACAVCAPS mid manifest is missing or empty: $MID_MANIFEST" >&2
  exit 1
fi
...
echo "========== ACAVCAPS HUGINN AUDIO SWIFT MID TRAIN =========="
echo "mode=lora_llm generator_frozen_audio_encoder aligner_trainable"
echo "dataset=$MID_MANIFEST"
...
swift sft \
  ...
  --dataset "$MID_MANIFEST" \
  --max_length 192 \
  --output_dir outputs/huginn_audio_acavcaps_caption_mid_generator \
  --tuner_type lora_llm \
