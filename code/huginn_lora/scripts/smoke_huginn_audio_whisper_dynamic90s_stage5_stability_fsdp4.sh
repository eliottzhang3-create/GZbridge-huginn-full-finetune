...
MAX_STEPS=20
RUN_ROOT="${HUGINN_AUDIO_DYNAMIC90S_STAGE5_RUN_ROOT:-outputs/huginn_audio_whisper_dynamic90s_stage5_stability_fsdp4/run-$(date +%Y%m%d_%H%M%S)}"
if [ -e "$RUN_ROOT" ]; then
  echo "Stage 5 run root already exists; choose a fresh HUGINN_AUDIO_DYNAMIC90S_STAGE5_RUN_ROOT: $RUN_ROOT" >&2
  exit 1
fi
mkdir -p "$RUN_ROOT"
...
MARKER_INSPECTOR="$REPO_ROOT/code/huginn_lora/scripts/inspect_huginn_audio_whisper_dynamic90s_stage5_markers.py"

for required_path in "$MODEL_PATH" "$PLUGIN_PATH" "$PREPARE_SCRIPT" "$MARKER_INSPECTOR"; do
  if [ ! -e "$required_path" ]; then
    echo "Required Stage 5 path is missing: $required_path" >&2
    exit 1
  fi
done
...
if missing:
    raise SystemExit(f'Installed Swift lacks required Stage 5 arguments: {missing}')
print('[precheck] swift_stage5_arguments=present')
PY
...
if [ ! -s "$TRAIN_MANIFEST" ]; then
  echo "Stage 5 synthetic manifest is missing or empty: $TRAIN_MANIFEST" >&2
  exit 1
fi
...
export HUGINN_AUDIO_DYNAMIC90S_STAGE5_AUDIT_DIR="$AUDIT_DIR"
export HUGINN_AUDIO_DYNAMIC90S_STAGE5_MAX_STEPS="$MAX_STEPS"

echo "========== HUGINN WHISPER DYNAMIC90S STAGE 5 FSDP4 STABILITY =========="
...
echo "max_steps=$MAX_STEPS expected_optimizer_updates=$MAX_STEPS save_strategy=no"
echo "stage6_checkpoint_save_reload=not_included"

TRAIN_PID=""
MONITOR_PID=""

print_resource_snapshot() {
  echo "========== DYNAMIC90S STAGE 5 RESOURCE SNAPSHOT =========="
...
on_exit() {
  ...
  trap - EXIT
  stop_resource_monitor
  echo "========== HUGINN WHISPER DYNAMIC90S STAGE 5 EXIT =========="
...
on_signal() {
  signal_name=$1
  echo "========== HUGINN WHISPER DYNAMIC90S STAGE 5 SIGNAL =========="
...
echo "========== HUGINN WHISPER DYNAMIC90S STAGE 5 STABILITY PASSED =========="
