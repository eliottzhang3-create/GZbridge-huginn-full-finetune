...
OUTPUT_DIR="outputs/huginn_whisper_dynamic30s_multiplier_checkpoint20000_clotho_caption_samples"
...
vc submit \
  ...
  -c 8 -m 32G -g 1 \
  -n 1 \
  -j generate-whisper-dynamic30s-multiplier-20000-4090-$(date +%m%d%H%M) \
  -d "$SCRIPT_DIR" \
  JOB=1:1 "$SCRIPT_DIR/log/generate_clotho_caption_samples_huginn_audio_whisper_dynamic30s_multiplier_checkpoint20000_4090.JOB.log" \
