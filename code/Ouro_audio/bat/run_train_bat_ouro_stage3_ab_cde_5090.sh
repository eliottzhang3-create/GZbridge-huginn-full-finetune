...
vc submit \
  ...
  --cmd "BAT_STAGE3_AB_CDE_MANIFEST=$(printf '%q' "$MANIFEST") BAT_STAGE3_AB_CDE_REPORT=$(printf '%q' "$REPORT") BAT_STAGE3_AB_CDE_OUTPUT_DIR=$(printf '%q' "$OUTPUT_DIR") BAT_LAUNCH_MODE=ddp bash scripts/train_bat_ouro_stage3_ab_cde_remote.sh"
