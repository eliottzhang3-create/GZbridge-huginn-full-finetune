...
vc submit \
  ...
  --cmd "BAT_TOKEN_AUDIT_MANIFEST=$(printf '%q' "$MANIFEST") BAT_TOKEN_AUDIT_OUTPUT_REPORT=$(printf '%q' "$OUTPUT_REPORT") BAT_TOKEN_AUDIT_PROGRESS_EVERY=$(printf '%q' "$PROGRESS_EVERY") bash scripts/audit_bat_manifest_token_lengths_remote.sh"
