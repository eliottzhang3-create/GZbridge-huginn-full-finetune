...
for name in OURO_MODEL_PATH OURO_BAT_PLUGIN_PATH OURO_BAT_DATALOADER_START_INDEX OURO_BAT_DATALOADER_WARMUP_BATCHES OURO_BAT_DATALOADER_TOTAL_BATCHES OURO_BAT_DATALOADER_PREFETCH_FACTOR OMP_NUM_THREADS MKL_NUM_THREADS OPENBLAS_NUM_THREADS; do
  value="${!name:-}"
  if [ -n "$value" ]; then
