...
STAMP=$(date +%m%d%H%M)
JOB_NAME="bat-ouro-eval-${BAT_EVAL_TYPE//[^A-Za-z0-9]/_}-${STAMP}"
...
for name in BAT_EVAL_MODEL_PATH BAT_EVAL_CHECKPOINT BAT_EVAL_QA_ROOT BAT_EVAL_AUDIO_ROOT BAT_EVAL_REVERB_ROOT BAT_EVAL_SPATIAL_AST_ROOT BAT_EVAL_SPATIAL_AST_CHECKPOINT BAT_EVAL_QFORMER_SOURCE BAT_EVAL_START_INDEX BAT_EVAL_MAX_RECORDS BAT_EVAL_MAX_NEW_TOKENS BAT_EVAL_NUM_BEAMS BAT_EVAL_RIR_POLICY BAT_EVAL_BINARY_ANSWER_PROMPT BAT_EVAL_DETECTION_MODE BAT_EVAL_LABEL_CSV BAT_EVAL_LABEL_EMBEDDINGS BAT_EVAL_EMBEDDING_MODEL BAT_EVAL_OPENAI_API_KEY_ENV BAT_EVAL_OVERWRITE; do
  append_env "$name"
done
...
vc submit \
  ...
  JOB=1:1 "${LOG_PATH}" \
  --cmd "cd ${REPO} && ${CMD_PREFIX}bash code/Ouro_audio/bat/scripts/eval_bat_ouro_online_remote.sh"
