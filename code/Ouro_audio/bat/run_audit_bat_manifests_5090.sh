...
vc submit \
  ...
  --cmd "BAT_STAGE1_MANIFEST=$(printf '%q' "$STAGE1") BAT_STAGE2_MANIFEST=$(printf '%q' "$STAGE2") BAT_STAGE3_MANIFEST=$(printf '%q' "$STAGE3") BAT_MANIFEST_AUDIT_ROOT=$(printf '%q' "$AUDIT_ROOT") bash scripts/audit_bat_manifests_remote.sh"
