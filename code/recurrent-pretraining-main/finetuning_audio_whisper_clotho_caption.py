"""Single-GPU Clotho caption training for the Huginn audio experiment branch."""
...
@dataclass
class CLISettings:
    run_name: str = "huginn-audio-whisper-clotho-caption-v1"
    ...
    def __getitem__(self, idx):
        ...
        for offset in range(len(self.examples)):
            ...
            try:
                ...
                last_error = exc
                print(f"[audio-caption][bad-audio] skip path={audio_path} error={type(exc).__name__}: {exc}")
                continue
...
def print_trainable_summary(model):
    ...
    print("[audio-caption] trainable parameter tensors:")
    for name, shape in trainable:
        print(f"  - {name}: {shape}")
    print(f"[audio-caption] frozen_param_tensors={frozen_count} trainable_param_tensors={len(trainable)}")
...
def print_grad_summary(model):
    ...
    print("[audio-caption] grad summary:")
...
def rotate_checkpoints(output_dir: Path, keep_last: int):
    ...
    while len(checkpoints) > keep_last:
        ...
        for child in oldest.iterdir():
            child.unlink()
        oldest.rmdir()
        print(f"[audio-caption] removed old checkpoint {oldest}")
...
def save_trainable_state(model, save_dir: Path, step: int, keep_last: int):
    ...
    print(f"[audio-caption] saved trainable checkpoint to {checkpoint_dir}")
...
def main():
    ...
    print(f"[audio-caption] dataset_dir={cfg.dataset_dir}")
    print(f"[audio-caption] init_checkpoint_dir={cfg.init_checkpoint_dir}")
    print(f"[audio-caption] target_sample_rate={cfg.target_sample_rate} max_audio_seconds={cfg.max_audio_seconds}")
    print(f"[audio-caption] use_randomized_num_steps={cfg.use_randomized_num_steps}")
    ...
    print(f"[audio-caption] backbone load missing={len(base_load.missing_keys)} unexpected={len(base_load.unexpected_keys)}")
    if base_load.unexpected_keys:
        print("[audio-caption] unexpected_keys:", base_load.unexpected_keys[:10])
    if base_load.missing_keys:
        print("[audio-caption] first_missing_keys:", base_load.missing_keys[:10])

    init_state_path = Path(cfg.init_checkpoint_dir) / "trainable_state.pt"
    init_state = torch.load(init_state_path, map_location="cpu")
    delta_load = model.load_state_dict(init_state, strict=False)
    print(f"[audio-caption] delta load missing={len(delta_load.missing_keys)} unexpected={len(delta_load.unexpected_keys)}")
    if delta_load.unexpected_keys:
        print("[audio-caption] unexpected_delta_keys:", delta_load.unexpected_keys[:10])
    ...
    print(f"[audio-caption] model_dtype={model_dtype}")

    trainable_params = [param for param in model.parameters() if param.requires_grad]
    optimizer = torch.optim.AdamW(trainable_params, lr=cfg.lr)

    dataset = ClothoCaptionDataset(cfg.dataset_dir, cfg.train_jsonl, cfg.target_sample_rate, cfg.max_audio_seconds)
    print(f"[audio-caption] dataset_size={len(dataset)}")
    print(f"[audio-caption] first_audio_path={dataset[0]['audio_path']}")
    ...
    print(
        f"[audio-caption] micro_batch_size={cfg.micro_batch_size} optimizer_update_every=1_micro_step "
    ...
    for epoch in range(1, cfg.epochs + 1):
        print(f"[audio-caption] epoch={epoch} begin")
        for batch in dataloader:
            ...
            if not torch.isfinite(loss):
                raise RuntimeError(f"Non-finite loss in Clotho caption training at step={step}: {loss}")
            loss.backward()
            ...
            print(
                f"[audio-caption] epoch={epoch} step={step} loss={loss.item():.4f} "
    ...
    print("--------------------------------------------------------------------")
    print(f"[audio-caption] completed steps={step} elapsed={elapsed:.2f}s")
    if device.type == "cuda":
        print(
            f"[audio-caption] max_mem_allocated={torch.cuda.max_memory_allocated(device) / float(1024**3):.3f} GB "
