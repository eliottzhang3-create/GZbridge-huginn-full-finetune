...
RUN_TAG="${HRM_AUDIO_WRAPPER_RUN_TAG:-$(date +%Y%m%d-%H%M%S)}"
RUN_DIR="${HRM_AUDIO_WRAPPER_RUN_DIR:-$REPO_ROOT/outputs/hrm_text/audio_wrapper_inspect/$RUN_TAG}"
OUTPUT_REPORT="${HRM_AUDIO_WRAPPER_OUTPUT_REPORT:-$RUN_DIR/wrapper_inspect.json}"

echo "========== INSPECT HRM AUDIO WRAPPER =========="
...
python -u code/HRM_Audio/scripts/inspect_hrm_audio_wrapper.py \
